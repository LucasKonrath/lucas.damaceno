...
public class PaisDaoImpl implements PaisDao {
    ...
    public void insert(Pais pais) {
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(INSERT_PAIS)) {
    ...
    public void update(Pais pais) {
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(UPDATE_PAIS)) {
    ...
    public void delete(Pais pais) {
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(DELETE_PAIS)) {
            preparedStatement.setLong(1, pais.getId());
            preparedStatement.executeUpdate();
        } catch (final SQLException e) {
            System.err.format("SQLException: %s", e);
    ...
    public Pais loadBy(Long id) {
        ...
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(LOAD_PAIS)) {
