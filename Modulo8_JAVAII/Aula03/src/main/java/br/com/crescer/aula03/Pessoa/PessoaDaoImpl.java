...
public class PessoaDaoImpl implements PessoaDao {
    ...
    public void insert(Pessoa pessoa) {
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(INSERT_PESSOA)) {
    ...
    public void update(Pessoa pessoa) {
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(UPDATE_PESSOA)) {
    ...
    public void delete(Pessoa pessoa) {
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(DELETE_PESSOA)) {
            preparedStatement.setLong(1, pessoa.getId());
            preparedStatement.executeUpdate();
        } catch (final SQLException e) {
            System.err.format("SQLException: %s", e);
    ...
    public Pessoa loadBy(Long id) {
        ...
        try (final PreparedStatement preparedStatement
                = ConnectionUtils.retornaConexao().prepareStatement(LOAD_PESSOA)) {
